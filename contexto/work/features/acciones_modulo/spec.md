...
- UI “Acciones” con tabs (en `/actions`):
  ...
  - **Templates**: plantillas de reglas (crear/editar/versionar/asignar) con create/edit via modal.
- Rutas:
...
- `/actions/templates/:id`
  - Vista de consulta/auditoria (read-only en v1).
  - Tabs sugeridos: Definition (Scratch), Versions, Assignments, Runs (fallos)
  - La edicion/creacion de versiones se realiza desde modal en `/actions` -> tab Templates.
