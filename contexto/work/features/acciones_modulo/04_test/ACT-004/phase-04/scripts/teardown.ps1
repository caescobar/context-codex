...
Add-Content -Path $CommandsLog -Value ("{0} | teardown phase-04 (safe no-op), DRY_RUN={1}" -f (Get-Date -Format s), $DryRun)
...
Add-Content -Path $OutputsLog -Value ("{0} | Observed: cierre operativo -> si se levantaron instancias para QA, deben apagarse y verificarse detenidas." -f (Get-Date -Format s))
Add-Content -Path $NotesFile -Value "- Teardown phase-04: no se realizaron cambios destructivos."
Add-Content -Path $NotesFile -Value "- Regla de cierre: toda instancia levantada durante QA debe apagarse y verificarse como detenida."
