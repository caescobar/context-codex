...
log_output "Expected: run validates ACT-006 phase-03 frontend runs tab + typed contracts + no-regression."
...
summary_file="$REPO_ROOT/contexto/work/features/acciones_modulo/03_execution/ACT-006/phase-03.md"
global_runs_endpoint="$REPO_ROOT/telemetric-api/src/Telemetric.Api/Features/Actions/Runs/GetRuns/GetRunsEndpoint.cs"
...
for file in \
  ...
  "$menu_file" \
  "$summary_file" \
  "$global_runs_endpoint" \
  "$template_runs_endpoint"; do
...
contains_text "$summary_file" "tab \`Runs\`" && log_output "Observed: phase_summary_runs_tab=true" || log_output "Observed: phase_summary_runs_tab=false"
contains_text "$view_file" "<v-tab value=\"runs\">Runs</v-tab>" && log_output "Observed: view_runs_tab_present=true" || log_output "Observed: view_runs_tab_present=false"
contains_text "$view_file" "activeTab = ref<'runs' | 'templates'>('runs')" && log_output "Observed: view_active_tab_default_runs=true" || log_output "Observed: view_active_tab_default_runs=false"
...
contains_text "$view_file" "No se pudieron cargar las ejecuciones." && log_output "Observed: view_runs_error_copy=true" || log_output "Observed: view_runs_error_copy=false"
contains_text "$view_file" "runsTotalItems === 0" && log_output "Observed: view_runs_empty_state=true" || log_output "Observed: view_runs_empty_state=false"
contains_text "$view_file" "UiDynamicFilter" && log_output "Observed: view_filter_present=true" || log_output "Observed: view_filter_present=false"
contains_text "$view_file" "UiServerTable" && log_output "Observed: view_table_present=true" || log_output "Observed: view_table_present=false"
...
contains_text "$service_file" "getRuns:" && log_output "Observed: service_get_runs_method=true" || log_output "Observed: service_get_runs_method=false"
contains_text "$service_file" "/actions/runs" && log_output "Observed: service_get_runs_route=true" || log_output "Observed: service_get_runs_route=false"
contains_text "$types_file" "export type ActionRunsQueryParams" && log_output "Observed: types_query_params=true" || log_output "Observed: types_query_params=false"
contains_text "$types_file" "export type ActionRunListItem" && log_output "Observed: types_run_list_item=true" || log_output "Observed: types_run_list_item=false"
contains_text "$types_file" "export type ActionRunStatus" && log_output "Observed: types_run_status=true" || log_output "Observed: types_run_status=false"
contains_text "$types_file" "export type ActionRunContext" && log_output "Observed: types_run_context=true" || log_output "Observed: types_run_context=false"
...
contains_text "$global_runs_endpoint" "Get(\"/api/v1/actions/runs\")" && log_output "Observed: backend_global_runs_route=true" || log_output "Observed: backend_global_runs_route=false"
contains_text "$template_runs_endpoint" "Get(\"/api/v1/actions/templates/{RuleTemplateId}/runs\")" && log_output "Observed: backend_template_runs_route=true" || log_output "Observed: backend_template_runs_route=false"
contains_text "$global_runs_endpoint" "Policies(PermissionClaims.Actions.View)" && log_output "Observed: backend_global_runs_policy=true" || log_output "Observed: backend_global_runs_policy=false"
contains_text "$template_runs_endpoint" "Policies(PermissionClaims.Actions.View)" && log_output "Observed: backend_template_runs_policy=true" || log_output "Observed: backend_template_runs_policy=false"
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  required_checks=(
    ...
    "service_get_runs_method"
    "service_get_runs_route"
    "types_query_params"
    "types_run_list_item"
    "types_run_status"
    "types_run_context"
    ...
    "backend_global_runs_route"
    "backend_template_runs_route"
    "backend_global_runs_policy"
    "backend_template_runs_policy"
  )
