...
Write-OutputLog "Expected: setup validates prerequisites and discovery for ACT-006 phase-03."
...
$summaryRel = "contexto/work/features/acciones_modulo/03_execution/ACT-006/phase-03.md"
...
try {
    Invoke-Step "rg --line-number -F -e ""activeTab = ref<'runs' | 'templates'>('runs')"" -e ""<v-tab value=`"runs`">Runs</v-tab>"" telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'UiDynamicFilter' -e 'UiServerTable' -e 'runsErrorMessage' -e 'No se pudieron cargar las ejecuciones.' telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e ""item.status === 'Fail'"" -e 'Fallo sin detalle de error.' telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'getRuns:' -e '/actions/runs' telemetric-front/src/features/actions/actions.service.ts"
    Invoke-Step "rg --line-number -F -e 'ActionRunsQueryParams' -e 'ActionRunListItem' -e 'ActionRunStatus' -e 'ActionRunContext' telemetric-front/src/features/actions/types.ts"
