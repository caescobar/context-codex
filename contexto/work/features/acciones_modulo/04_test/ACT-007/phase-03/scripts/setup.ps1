...
Write-OutputLog "Expected: setup validates prerequisites and discovery for ACT-007 phase-03."
...
$summaryRel = "contexto/work/features/acciones_modulo/03_execution/ACT-007/phase-03.md"
$viewRel = "telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
...
$routesRel = "telemetric-front/src/features/actions/actions.routes.ts"
$menuRel = "telemetric-front/src/layouts/menuItems.ts"
...
try {
    Invoke-Step "rg --line-number -F -e `"activeTab = ref<`" -e `"<v-tab value=`"rules`">Rules</v-tab>`" telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'UiDynamicFilter' -e 'UiServerTable' -e 'rulesErrorMessage' -e 'No se pudieron cargar las reglas.' telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'hasLastAttemptFail' -e 'Ultimo fail' -e 'Fail sin detalle de error.' telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'updateRuleState(item, !item.isPaused)' -e 'Actions.Update' telemetric-front/src/features/actions/views/ActionsTemplatesView.vue"
    Invoke-Step "rg --line-number -F -e 'getRules:' -e 'updateRuleState:' -e '/actions/rules' telemetric-front/src/features/actions/actions.service.ts"
    Invoke-Step "rg --line-number -F -e 'ActionRuleListItem' -e 'ActionRulesQueryParams' -e 'UpdateRuleStateRequest' -e 'UpdateRuleStateResponse' -e 'RuleOperationalStatus' telemetric-front/src/features/actions/types.ts"
    Invoke-Step "rg --line-number -F -e 'path: ''/actions''' -e 'requiresPermission: ''Actions.View''' telemetric-front/src/features/actions/actions.routes.ts telemetric-front/src/layouts/menuItems.ts"
}
finally {
    Pop-Location
}
