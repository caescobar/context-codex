...
Write-OutputLog "Expected: run validates ACT-007 phase-01 discovery/equivalence and OpenAPI contract for Rules."
...
$phaseSummaryFile = Join-Path $RepoRoot "contexto/work/features/acciones_modulo/03_execution/ACT-007/phase-01.md"

$hasSummary = Test-Contains $phaseSummaryFile "ausencia de endpoints"
$hasRulesGlobal = Test-Contains $openapiFile "/api/v1/actions/rules:"
$hasRulesByDevice = Test-Contains $openapiFile "/api/v1/actions/devices/{deviceId}/rules:"
$hasRuleStatePatch = Test-Contains $openapiFile "/api/v1/actions/rules/{ruleInstanceId}/state:"
$hasViewPolicy = Test-Contains $openapiFile "x-required-policy: Actions.View"
$hasUpdatePolicy = Test-Contains $openapiFile "x-required-policy: Actions.Update"
$hasRulesResponse = Test-Contains $openapiFile "GetRulesResponse"
$hasRuleListItem = Test-Contains $openapiFile "RuleListItem"
$hasOperationalStatus = Test-Contains $openapiFile "RuleOperationalStatus"
$hasUpdateRequest = Test-Contains $openapiFile "UpdateRuleStateRequest"
$hasUpdateResponse = Test-Contains $openapiFile "UpdateRuleStateResponse"
$hasFieldRuleInstanceId = Test-Contains $openapiFile "ruleInstanceId:"
$hasFieldIsPaused = Test-Contains $openapiFile "isPaused:"
$hasFieldOperationalStatus = Test-Contains $openapiFile "operationalStatus:"
$hasFieldLastFail = Test-Contains $openapiFile "hasLastAttemptFail:"
$hasFieldLastStatus = Test-Contains $openapiFile "lastAttemptStatus:"
$hasFieldLastAttemptedAt = Test-Contains $openapiFile "lastAttemptedAt:"
$hasClaimView = Test-Contains $claimsFile "Actions.View"
$hasClaimUpdate = Test-Contains $claimsFile "Actions.Update"

Write-OutputLog "Observed: phase_summary_note=$hasSummary"
Write-OutputLog "Observed: openapi_rules_global=$hasRulesGlobal"
Write-OutputLog "Observed: openapi_rules_by_device=$hasRulesByDevice"
Write-OutputLog "Observed: openapi_rules_patch_state=$hasRuleStatePatch"
Write-OutputLog "Observed: openapi_policy_actions_view=$hasViewPolicy"
Write-OutputLog "Observed: openapi_policy_actions_update=$hasUpdatePolicy"
Write-OutputLog "Observed: openapi_schema_get_rules_response=$hasRulesResponse"
Write-OutputLog "Observed: openapi_schema_rule_list_item=$hasRuleListItem"
Write-OutputLog "Observed: openapi_schema_rule_operational_status=$hasOperationalStatus"
Write-OutputLog "Observed: openapi_schema_update_rule_state_request=$hasUpdateRequest"
Write-OutputLog "Observed: openapi_schema_update_rule_state_response=$hasUpdateResponse"
Write-OutputLog "Observed: payload_field_ruleInstanceId=$hasFieldRuleInstanceId"
Write-OutputLog "Observed: payload_field_isPaused=$hasFieldIsPaused"
Write-OutputLog "Observed: payload_field_operationalStatus=$hasFieldOperationalStatus"
Write-OutputLog "Observed: payload_field_hasLastAttemptFail=$hasFieldLastFail"
Write-OutputLog "Observed: payload_field_lastAttemptStatus=$hasFieldLastStatus"
Write-OutputLog "Observed: payload_field_lastAttemptedAt=$hasFieldLastAttemptedAt"
Write-OutputLog "Observed: claims_actions_view=$hasClaimView"
Write-OutputLog "Observed: claims_actions_update=$hasClaimUpdate"

Push-Location $RepoRoot
try {
    $equivalenceMatches = @(Invoke-Step "rg --line-number -g '*Endpoint.cs' -F -e '/api/v1/actions/rules`"' -e '/api/v1/actions/devices/{deviceId}/rules`"' -e '/api/v1/actions/rules/{ruleInstanceId}/state`"' telemetric-api/src/Telemetric.Api/Features/Actions")
    if ($DryRun -eq "0") {
        if ($equivalenceMatches.Count -eq 0) {
            Write-OutputLog "Observed: backend_rules_equivalence=PASS (no equivalent endpoint found)."
        }
        else {
            Write-OutputLog "Observed: backend_rules_equivalence=FAIL (unexpected matches found)."
            Add-Content -Path $OutputsLog -Value ($equivalenceMatches | Out-String).TrimEnd()
            throw "Equivalent Rules endpoint found in backend."
        }
    }
    else {
        Write-OutputLog "Observed (pending): backend_rules_equivalence requires DRY_RUN=0 execution."
    }

    $viewPolicyMatches = @(Invoke-Step "rg --line-number -F -e `"x-required-policy: Actions.View`" contexto/openapi/actions.yaml")
    $updatePolicyMatches = @(Invoke-Step "rg --line-number -F -e `"x-required-policy: Actions.Update`" contexto/openapi/actions.yaml")
    if ($DryRun -eq "0") {
        if ($viewPolicyMatches.Count -eq 0) { throw "Missing x-required-policy: Actions.View in OpenAPI." }
        if ($updatePolicyMatches.Count -eq 0) { throw "Missing x-required-policy: Actions.Update in OpenAPI." }
    }
