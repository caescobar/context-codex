...
Write-OutputLog "Expected: run valida criterios ACT-005 phase-04 (customer DSL, overrides v1, permisos, no-regresion)."
...
$phaseSummaryFile = Join-Path $RepoRoot "contexto/work/features/acciones_modulo/03_execution/ACT-005/phase-04.md"

$hasSummaryContract = Test-Contains $phaseSummaryFile "contrato DSL canonico"
$hasRoute = Test-Contains $routesFile "path: '/my-devices/:id/edit'"
$hasCanAssign = Test-Contains $viewFile "permissions?.includes('Actions.Assign')"
$hasPermissionMessage = Test-Contains $viewFile "No tienes permiso"
$hasValidateDefinition = Test-Contains $viewFile "const validateAndBuildDefinition"
$hasBuildOverrides = Test-Contains $viewFile "const buildOverrides"
...
$hasRecipientEmailRule = Test-Contains $viewFile "Email invalido:"
$hasThresholdOverrideRule = Test-Contains $viewFile "threshold override invalido."
$hasEmailOverrideRule = Test-Contains $viewFile "Email override invalido:"
$hasOverrideThreshold = Test-Contains $viewFile "overrides.threshold = threshold"
$hasOverrideEmail = Test-Contains $viewFile "overrides.email = { recipients }"
$hasCreateFromDeviceCall = Test-Contains $viewFile "actionsService.createRuleFromDevice"
$serviceHasCreateFromDevice = Test-Contains $serviceFile "createRuleFromDevice"
$serviceHasCreateFromDeviceEndpoint = Test-Contains $serviceFile "/create-from-device"
$serviceHasDefinitionSerialization = Test-Contains $serviceFile "JSON.stringify(payload.definitionJson)"
$serviceHasOverridesSerialization = Test-Contains $serviceFile "JSON.stringify(payload.overridesJson)"
$typesHasRuleDefinition = Test-Contains $typesFile "definitionJson: RuleDefinitionV1"
$typesHasOverridesType = Test-Contains $typesFile "overridesJson?: RuleInstanceOverridesV1 | null"
$typesHasRequestType = Test-Contains $typesFile "export interface CreateRuleFromDeviceRequest"
$openApiHasPath = Test-Contains $openApiFile "/api/v1/actions/assignments/create-from-device:"
$openApiHasDefinitionV1 = Test-Contains $openApiFile "DefinitionJsonV1"
$openApiHasOverridesV1 = Test-Contains $openApiFile "RuleInstanceOverridesV1"

Write-OutputLog "Observed: execution_summary_contract_trace=$hasSummaryContract"
Write-OutputLog "Observed: route_my_devices_edit_exists=$hasRoute"
Write-OutputLog "Observed: ui_permission_gate_assign=$hasCanAssign"
Write-OutputLog "Observed: ui_permission_block_message=$hasPermissionMessage"
Write-OutputLog "Observed: ui_builder_definition=$hasValidateDefinition"
Write-OutputLog "Observed: ui_builder_overrides=$hasBuildOverrides"
Write-OutputLog "Observed: rule_type_coverage=$($hasRuleType1 -and $hasRuleType2 -and $hasRuleType3 -and $hasRuleType4 -and $hasRuleType5)"
...
Write-OutputLog "Observed: recipients_email_validation=$hasRecipientEmailRule"
Write-OutputLog "Observed: overrides_threshold_validation=$hasThresholdOverrideRule"
Write-OutputLog "Observed: overrides_email_validation=$hasEmailOverrideRule"
Write-OutputLog "Observed: overrides_limited_scope=$($hasOverrideThreshold -and $hasOverrideEmail)"
Write-OutputLog "Observed: ui_calls_create_rule_from_device=$hasCreateFromDeviceCall"
Write-OutputLog "Observed: service_create_from_device_wiring=$($serviceHasCreateFromDevice -and $serviceHasCreateFromDeviceEndpoint)"
Write-OutputLog "Observed: service_backend_compat_serialization=$($serviceHasDefinitionSerialization -and $serviceHasOverridesSerialization)"
Write-OutputLog "Observed: typed_contracts_create_from_device=$($typesHasRuleDefinition -and $typesHasOverridesType -and $typesHasRequestType)"
Write-OutputLog "Observed: openapi_dsl_contract=$($openApiHasPath -and $openApiHasDefinitionV1 -and $openApiHasOverridesV1)"
