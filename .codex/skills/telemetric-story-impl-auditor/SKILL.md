...
- `.codex/skills/telemetric-frontend-standards/SKILL.md` (MANDATORY if story type includes FE OR any planned file path starts with `telemetric-front/`)
...
- Route meta + permissions + menu parity
- Admin/devices UX parity (breadcrumb + filter + server table)
- Typed services + stable list/paged contracts
- Encoding hygiene (UTF-8)
- Typecheck no-regression gate (exclude `src/_demo/**`)
Backend:
...
**Operational Lock (QA artifacts path):**
- QA packs/checklists/scripts/evidence MUST live under:
...
- If repo contains any legacy QA paths (e.g. `contexto/05_tests/`), treat them as deprecated and do not use them.
- Evidence (repo): locate existing `04_test` folders or confirm none exist.
...
- **Standard (source):** `telemetric-backend-standards` or `telemetric-sqlserver-standards` or telemetric-frontend-standards
...
- QA Pack vs 5-file rule:
  - Si una fase requiere generar QA pack completo (scripts ps1+sh + evidence + checklist),
    el auditor DEBE planificarlo en una fase dedicada o partirlo en 2 fases para respetar max 5 archivos.
---
